...
class SwipingViewController: UIViewController {
    ...
    func wasDragged(gestureRecognizer: UIPanGestureRecognizer) {
        ...
        var stretchAndRotation = rotation.scaleBy(x: scale, y: scale)
        ...
        if gestureRecognizer.state == UIGestureRecognizerState.ended {
            ...
            stretchAndRotation = rotation.scaleBy(x: 1, y: 1)
    ...
    func updateImage() {
        ...
        if let latitude = PFUser.current()?["location"].latitude {
            
            if let longitude = PFUser.current()?["location"].longitude {
